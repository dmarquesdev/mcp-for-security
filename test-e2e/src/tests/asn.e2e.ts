...
describe("ASN mapping", () => {
  describe("asnmap", () => {
    it("resolves ASN to CIDR ranges", { timeout: 60000 }, async () => {
      const skip = await shouldSkip(TestCategory.CREDENTIAL);
      if (skip) return;
      if (!process.env.PDCP_API_KEY) return;
      if (!(await isServiceHealthy("asnmap"))) return;
    ...
    it("resolves IP to ASN info", { timeout: 60000 }, async () => {
      const skip = await shouldSkip(TestCategory.CREDENTIAL);
      if (skip) return;
      if (!process.env.PDCP_API_KEY) return;
      if (!(await isServiceHealthy("asnmap"))) return;
    ...
    it("resolves domain to network ranges", { timeout: 60000 }, async () => {
      const skip = await shouldSkip(TestCategory.CREDENTIAL);
      if (skip) return;
      if (!process.env.PDCP_API_KEY) return;
      if (!(await isServiceHealthy("asnmap"))) return;
    ...
    it("resolves org name", { timeout: 60000 }, async () => {
      const skip = await shouldSkip(TestCategory.CREDENTIAL);
      if (skip) return;
      if (!process.env.PDCP_API_KEY) return;
      if (!(await isServiceHealthy("asnmap"))) return;
