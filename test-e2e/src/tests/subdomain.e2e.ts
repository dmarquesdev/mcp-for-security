...
describe("subdomain enumeration", () => {
  describe("subfinder", () => {
    it("finds subdomains for tesla.com", { timeout: 90000 }, async () => {
      const skip = await shouldSkip(TestCategory.PUBLIC);
      if (skip) return;
      if (!(await isServiceHealthy("subfinder"))) return;
  ...
  describe("assetfinder", () => {
    it("finds assets for tesla.com", { timeout: 90000 }, async () => {
      const skip = await shouldSkip(TestCategory.PUBLIC);
      if (skip) return;
      if (!(await isServiceHealthy("assetfinder"))) return;
  ...
  describe("crtsh", () => {
    it("returns certificate data for tesla.com", { timeout: 120000 }, async () => {
      const skip = await shouldSkip(TestCategory.PUBLIC);
      if (skip) return;
      if (!(await isServiceHealthy("crtsh"))) return;
  ...
  describe("amass", () => {
    it(`runs passive enumeration on ${TARGETS.EXAMPLE}`, { timeout: 120000 }, async () => {
      const skip = await shouldSkip(TestCategory.PUBLIC);
      if (skip) return;
      if (!(await isServiceHealthy("amass"))) return;
  ...
  describe("cero", () => {
    it(`probes ${TARGETS.EXAMPLE} certificates`, { timeout: 60000 }, async () => {
      const skip = await shouldSkip(TestCategory.PUBLIC);
      if (skip) return;
      if (!(await isServiceHealthy("cero"))) return;
  ...
  describe("github-subdomains", () => {
    it("finds subdomains via GitHub (credential required)", { timeout: 90000 }, async () => {
      const skip = await shouldSkip(TestCategory.CREDENTIAL);
      if (skip) return;
      if (!process.env.GITHUB_TOKEN) return;
      if (!(await isServiceHealthy("github-subdomains"))) return;
